...
public class TeamNotes extends Application {
	...
	// Script that allows us to get the selected (highlighted) text from an
	// HTMLEditor (Credit: https://gist.github.com/jewelsea/7819195)
	private static final String SELECT_TEXT = "(function getSelectionText() {\n" + "    var text = \"\";\n"
	...
	private Button initializeOpenButton(Stage stage, TabPane tabPane) {
		...
		open.setOnAction(event -> {
			...
			// Get the File the user selected and the contents into a single String then
			// create the tab. Also check if they did not choose a file
	...
	private Button initializeSaveButton(Stage stage, TabPane tabPane) {
		...
		save.setOnAction(event -> {
			// Get the File associated with the Tab. If the File does not exist, allow the
			...
			if (file == null) {
				...
				try {
					// Set the FileChooser to save dialog, and allow the user to enter the file they
					// want to create. Then check if the file ends with .html
			...
			// Check if the File is null (this is a special case if the user opens the
			// dialog menu then immediately presses escape)
			if (file == null) {
				return;
			}
	...
	// Method to encapsulate the parsing of Java code with Beanshell
	private void bsh(String selector, Tab current) throws EvalError {
		// BeanShell
		final Wrapper w = new Wrapper();
		final Interpreter i = new Interpreter(w);

		final StringBuilder builder = new StringBuilder("Input:\n> ").append(selector).append("\n\nOutput:\n> ");
		try {
			final Object obj = i.eval(selector);
			builder.append(obj);
	...
	// Updates the TabPanes closing policy depending on the number of tabs opened
	private void updateTabPolicy(TabPane tabPane) {
		tabPane.setTabClosingPolicy(tabPane.getTabs().size() <= 1 ? TabClosingPolicy.UNAVAILABLE : TabClosingPolicy.ALL_TABS);
	}
	
	// Adds a close listener to a tab that when closed checks if its the last
	// remaining file. If so it opens a new doc so there is always a doc open
	...
	private void closeAndReopenTab(TabPane tabPane, Tab tab, File file) {
		...
		// Create the new Tab and its HTML Editor then set the HTMLEditors text to the
	...
	public void start(Stage stage) {
		// Create the TabPane (and create blank tab so the GUI isnt empty when run) and
		// HBox pane (will be the button bar)
		final TabPane tabPane = new TabPane();
		createNewTab(tabPane);

		// Create the HBox which will act as the button bar, create all the buttons, and
		// add all the buttons to the bar (HBox)
		...
		// Create the root VBox pane, set the background color to white and add the
		// button bar/tab pane to the root pane
